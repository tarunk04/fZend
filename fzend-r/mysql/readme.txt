...
Read Me File
--------------
Mini Server 13 is a free product and does not contain commercial software!
...
Server uses the standard port 3313
...
--------------------------
This mysql server uses non standard port 3313 preventing it clashing with the standard port 3306.
To change the port two files need editing for example change port to 3307:

a) Edit mysql_start.bat locate line:
    udrive\bin\mysqladmin.exe --port=3313 --user=root --password=root shutdown
   Change to:
    udrive\bin\mysqladmin.exe --port=3307 --user=root --password=root shutdown

b) Edit  my.cnf located in folder \mini_server_13\udrive\bin
   Locate the lines:
     port=3313
   Change to:
     port=3307
...
To check server status type:
  mysqladmin --port=3313 --user=root --password=root status

The first thing you should do is set a new password (for example mpg123)
using the following command:
  mysqladmin --port=3313 --user=root --password=root password mpg123
...
To check the server status type:
  mysqladmin --port=3313 --user=root --password=mpg123 status
...
Log in to server and start a mysql prompt type:
 mysql --port=3313 --user=root --password=root
  OR if you changed the password:
 mysql --port=3313 --user=root --password=mpg123
